...
const User = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <View className='me-4' >
          <TouchableOpacity onPress={()=>router.push("/user/add-user")} className='flex flex-row justify-center items-center gap-2'>
  ...
  return (
    <SafeAreaView className='bg-white h-full'>
        <View className='flex flex-row justify-between rounded-full h-14 items-center px-5 m-2 bg-slate-200'>
          <TextInput placeholder='Search User' className='flex-1' value={searchQuery} onChangeText={setSearchQuery} />
        ...
        <ScrollView  refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} className=' bg-white'  >
          {
            ...
            data?.map((customer,index)=>
              <TouchableOpacity onPress={()=>router.push(`/user/${customer._id}`)} className='px-4 py-4 flex flex-row gap-3 items-center justify-between border-slate-200  border-b' key={index}>
                 <Image className='h-16 w-16 rounded-full' source={profile} />
                 {/* <View className='border border-primary p-3 rounded-full'>
                ...
                <View className='flex-1 flex gap-2 py-2 justify-between'>
                  ...
                  <View className='flex flex-row justify-between items-center '>
                     <View className='flex flex-row gap-2 items-center'>
                      <Ionicons name="at" size={18} color="#f2652d" />
                    <Text className='text-md font-pregular'>{customer.username }</Text>
                    </View>
