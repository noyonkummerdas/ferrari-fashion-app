...
const Customers = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <View className='me-4' >
          <TouchableOpacity onPress={()=>router.push("/customer/add-customer")} className='flex flex-row justify-center items-center gap-2'>
  ...
  return (
    <SafeAreaView className='bg-white h-full'>
        <View className='flex flex-row justify-between rounded-full h-14 items-center px-5 m-2 bg-slate-200'>
          <TextInput placeholder='Search Customer' className='flex-1' value={searchQuery} onChangeText={setSearchQuery} />
        ...
        <ScrollView  refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} className=' bg-white'  >
          {
            ...
            data.map((customer,index)=>
              <TouchableOpacity onPress={()=>router.push(`/customer/${customer._id}`)} className='px-4 py-4 flex flex-row gap-3 justify-between items-center border-slate-200  border-b' key={index}>
