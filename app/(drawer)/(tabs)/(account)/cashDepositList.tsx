...
const CashDepositList = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        <View className="me-4">
          <TouchableOpacity
            onPress={() => router.push("/(drawer)/(tabs)/(account)/cashDeposit")}
  ...
  useEffect(() => {
    console.log("CASH DEPOSIT DEBUG:", {
      hasWarehouse,
    ...
    if (data) {
      console.log("CASH DEPOSIT RAW DATA:", JSON.stringify(data).substring(0, 200));
    }
  ...
  const filteredList = depositList.filter((item) => {
    ...
    const dateMatch =
      item?.date && format(new Date(item.date), "dd-MM-yyyy").includes(term);
...
//   // API call for single date (backend safe)
//   const { data, isSuccess, isLoading, error, isError, refetch } =
//     useTransactionListQuery({
//       warehouse: userInfo?.warehouse,
//       type: "deposit",
...
//   // Store raw deposit list
//   const [depositList, setDepositList] = useState<any[]>([]);

//   useEffect(() => {
//     if (data) {
//       if (Array.isArray(data)) setDepositList(data);
//       else if (data && typeof data === "object" && "transactions" in data && Array.isArray(data.transactions)) {
//         setDepositList(data.transactions);
//       } else setDepositList([]);
//     }
//   }, [data, isSuccess]);
...
//                     })
//                   }
//                   className="flex-row justify-between bg-black-200 rounded-xl p-4 mx-4 mt-2 items-center"
//                 >
//                   <View className="flex flex-col items-start">
