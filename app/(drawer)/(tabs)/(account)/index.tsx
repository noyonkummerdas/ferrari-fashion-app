...
const Accounts = () => {
  ...
  // --- Sudo Balance ---
  const [sudoBalance, setSudoBalance] = useState(0);
  ...
  // --- Fetch all deposits ---
  const { data: depositData } = useTransactionListQuery({
    warehouse: userInfo?.warehouse,
    type: "deposit",
    forceRefetch: true,
  });
  ...
  // --- Fetch all cashouts ---
  const { data: cashOutData } = useTransactionListQuery({
    warehouse: userInfo?.warehouse,
    type: "cashOut",
    forceRefetch: true,
  });
  ...
  // --- Fetch payments ---
  const { data: paymentData } = useTransactionListQuery({
    warehouse: userInfo?.warehouse,
    type: "payment",
    forceRefetch: true,
  });
  ...
  const { data: receivedPaymentData } = useTransactionListQuery({
    warehouse: userInfo?.warehouse,
    type: "paymentReceived",
    forceRefetch: true,
  });

  // --- Calculate sudoBalance once ---
  useEffect(() => {
    const totalDeposit = depositData?.transactions?.reduce((sum, t) => sum + (t.amount || 0), 0) || 0;
    const totalCashOut = cashOutData?.transactions?.reduce((sum, t) => sum + (t.amount || 0), 0) || 0;
    setSudoBalance(totalDeposit - totalCashOut);
  }, [depositData, cashOutData]); // date change will NOT affect sudoBalance

  // --- Totals for other sections ---
  const totalDepositAmount = depositData?.transactions?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
  const totalCashOutAmount = cashOutData?.transactions?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
  const totalPaymentAmount = paymentData?.transactions?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
  const totalReceivedPaymentAmount = receivedPaymentData?.transactions?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
  ...
  return (
    <>
      <StatusBar style="light" backgroundColor="#000" />
      <ScrollView className="flex-1 bg-black-700 h-full" showsVerticalScrollIndicator={false}>
        <View className="mt-2">
          <Text className="text-white text-lg font-pbold mb-4">Transactions</Text>
          ...
          <View className="flex-row mb-4 w-full">
            <View className="flex-1 ml-2 bg-black-200 rounded-xl h-28 p-4">
              ...
              <Text className="text-white text-xl font-pbold">{sudoBalance}</Text>
            </View>
          </View>
