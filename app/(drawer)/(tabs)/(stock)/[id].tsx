...
const StockDetails = () => {
  ...
  // Refetch when id or pathname changes
  useEffect(() => {
    refetch();
  }, [id, pathname]);

  // Reset redux stockItem on mount
  useEffect(() => {
    dispatch(resetStockItem());
    dispatch(clearError());
    dispatch(clearSuccess());
  ...
  // Set initial stock item in redux when data loads
  useEffect(() => {
    if (data && isSuccess) {
      dispatch(
        setStockItem({
  ...
  // ✅ Safe stock array
  const stockList = Array.isArray(data?.stock)
    ? data.stock.filter((item) => item && item.warehouse && item.warehouse.name)
    : [];
    ...
    <View className="flex-1 bg-white">
      ...
      <View className="bg-black flex-1 px-6 pt-8">
        ...
        <View className="flex-row justify-between mb-8">
          {/* Stock */}
          <View className="flex-1 mr-6">
            <Text className="text-yellow-500 text-lg font-pmedium mb-4">Stock</Text>
            <View className="flex-row items-center mb-3">
              <MaterialIcons name="inventory" size={24} color="#FDB714" style={{ marginRight: 8 }} />
              <Text className="text-white text-2xl font-pbold">
                {data?.currentStock ?? 0}
              </Text>
            </View>
          ...
          <View className="flex-1 ml-6">
            ...
            <View className="flex-row items-center">
              <MaterialIcons name="qr-code" size={24} color="#FDB714" style={{ marginRight: 8 }} />
              <Text className="text-white text-2xl font-pbold">{data?.code || "N/A"}</Text>
            </View>
          </View>
        </View>
        ...
        <ScrollView className="mb-32">
          {stockList.length === 0 ? (
            <Text className="text-gray-400 text-center">No stock history available</Text>
          ) : (
            stockList.map((item) => (
              <View
                key={item._id}
