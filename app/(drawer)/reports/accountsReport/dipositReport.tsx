...
export default function CashInReport() {
  ...
  // Set warehouses and default selectedWarehouse
  useEffect(() => {
    if (!warehousesData || !userInfo) return;

    setWarehouses(warehousesData);

    if (userInfo.type === "admin") {
      setSelectedWarehouse(warehousesData?.[0]?._id ?? null); // Admin first warehouse selected
    } else {
      setSelectedWarehouse(userInfo.warehouse ?? null); // Normal user sees own warehouse
    }
  }, [warehousesData, userInfo]);

  // Fetch cashDeposit data
  const { data: cashDepositData, refetch } = useTransactionListQuery({
  ...
  // Refetch when warehouse or date changes
  useEffect(() => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        <PrintButton
          filteredData={cashDepositData?.transactions || []}
          title="Cash Deposit Report"
        />
      ),
    });
  }, [navigation, cashDepositData]);

  const totalCashIn = cashDepositData?.transactions?.length || 0;
  const totalAmount =
    cashDepositData?.transactions?.reduce((sum, item) => sum + (item.amount || 0), 0) ||
    0;
  ...
  return (
    <>
      ...
      <View className="flex-1 bg-dark p-2">
        ...
        {/* List */}
        <FlatList
          data={cashDepositData?.transactions || []}
