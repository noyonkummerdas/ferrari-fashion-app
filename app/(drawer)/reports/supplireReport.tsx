...
const supplierReport = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      title: `${data?.name || "Supplier Report"}`,
      //@ts-ignore
      headerStyle: {
    ...
    <View className="flex-1 bg-dark">
      {/* Header */}
       <View className='flex-row items-center justify-between -2 mb-4'>
          ...
          <Dropdown
              data={supplierReport?.warehouses?.map(wh => ({ label: wh.name, value: wh._id })) || []}
              labelField="label"
              valueField="value"
      ...
      <View className="flex-row justify-between mb-4">
        ...
        <View className="bg-black-200 p-4 rounded-2xl w-[48%]">
          <Text className="text-zinc-300 text-sm">Total Purchases</Text>
