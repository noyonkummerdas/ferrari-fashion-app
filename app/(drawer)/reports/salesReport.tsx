...
console.log('salesReport',salesReport)
    ...
    navigation.setOptions({
      ...
      <View className='me-4 flex flex-row justify-center items-center gap-2' >
          <TouchableOpacity onPress={()=>console.log("download")} className='flex flex-row justify-center items-center gap-2'>
            <Ionicons name="download-outline" size={24} color="#fbd147" />
  ...
  }, [navigation, userInfo, filteredSales]);
      ...
      <View className="mt-4 p-4 bg-black-200 rounded-xl ">
        ...
        <Text className='text-gray-300 font-bold text-lg'>Product based sale</Text>
        {
          saleProductDetails.map((product, idx) => (
            <View key={idx} className="flex-row justify-between py-2 border-b border-gray-700">
              <Text className="text-gray-300">{product.name}</Text>
              <Text className="text-white">৳{product.balance?.toLocaleString()}</Text>
            </View>
          ))}
        </View>
        {/**customer wise sale */}
